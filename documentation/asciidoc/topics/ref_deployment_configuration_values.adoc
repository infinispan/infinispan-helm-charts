...
|`deploy.logging.console.json`
|Outputs logs as JSON on stdout instead of the default colored text format.
Useful for log aggregation pipelines such as ELK or Loki.
When enabled, each log line includes the following extra fields: `time`, `service`, `namespace`, and `pod`.
The `POD_NAME` and `POD_NAMESPACE` environment variables are automatically injected via the Kubernetes Downward API.
|`false`
